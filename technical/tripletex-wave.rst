...
---------------------------------
Every Tripletex Country will be synchronized with a Wave Country.

If a matching Wave Country already exists, the Tripletex Country will be merged with the existing one.
If no matching Wave Country is found, a new Wave Country will be created.

A Tripletex Country will merge with a Wave Country if one of the following property combinations match:
...
Once a link between a Tripletex Country and a Wave Country is established, it will keep in sync between the two systems, regardless of where it is edited.

The following properties are synchronized between a Tripletex Country and a Wave Country:
