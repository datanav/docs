.. _guide_create_first_synchronization_data_management:
...
In this section we we will cover the data management steps in a Sesam project. Data management in Sesam cover mapping imported data to the :ref:`global model <whatis-global>`, connecting this data to other imported data and :ref:`merging <merging-feature>` them. The final phase of data management in Sesam is the late schema binding, where we map global properties to the schema of the target system. 
|
|
...
.. panels::
    ...
    **Create your first synchronization:** This section includes 3 tutorials
    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        
    .. dropdown:: **1. Enrich data**
        :open:
