...
shapes of data which can be exposed and delivered to other systems. The entire system is driven by
...
Sesam produces and consumes streams of data, where each stream contains a number of data entities. Each entity consists of a
number of key-value pairs and with some special reserved property names. See the :doc:`entity data model <entitymodel>`
...
A key concept in Sesam is the *pipe* through which the data flows. A pipe consists of a source, an optional list of
...
*Sources* are configured to expose data as streams of entities from either external or internal sources. External sources consists of data streamed in from external providers, such as REST APIs or SQL databases.
Each external source is connected to a *System* which connects the pipe to the external provider, such as a web server hosting an
API endpoint or a SQL database. The job of the source is to convert the underlying data into a uniform representation; JSON, which is the format of data inside Sesam.
Some sources offer additional features, such as only exposing the entities that have changed to make support for change detection available.

Data from a source is piped through the transform into a dataset sink which writes
data into a named dataset. The dataset is the core storage mechanism in Sesam and consists of a log of entities with some
additional indexes to support lookups and joins. An entity is only appended to the dataset's log if the data is new
or has changed.

Datasets also act as sources (internal sources). One of the main uses of a dataset is as a source to a transformation. Transformations are
...
We use *Sesam* as the general name for a Sesam service instance. A given service instance exposes a single API endpoint and user interface. Internally, the service instance consists of configurations for all the pipe and system definitions, processes to execute the pumps and datasets for the storage of data.

A service instance is configured via the API, and the configuration in Sesam entity based. This means that we can track and understand if the configuration has changed in the same way we understand if any data has changed.

The API offers two ways to upload configurations. The first is via the 'config' endpoint. This allows a complete set of configuration to be uploaded and is typically used when bootstrapping a service instance in QA or production environments. The other way is to use the individual resources exposed via the API, such as a post to the collection of pipes.
...
A dataset is the basic means of storage inside the node. A dataset is a log of :doc:`entities <entitymodel>` supported by primary and secondary indexes. A *dataset sink* writes entities to the dataset. The dataset appends the entity to the log if, and only if, it is a new entity or if it is different from the most recent version of the same entity (:ref:`change tracking <concepts-change-tracking>`.
...
A *system* is any database or API that could be used as a source of data coming in to Sesam or as the target of entities coming out of Sesam. The system components provide a way to represent the actual systems being connected or integrated.

The system component has a couple of uses. It can be used to introspect the underlying system and provide back lists of possible 'source' or 'sink' targets. Often this information can be used on the command line or in the *Sesam Management Studio* to quickly and efficiently configure how the node consumes or delivers data. The other use of the *system* is that it allow configuration that may apply to many *source* definitions, e.g. connection strings, to be located and managed in one place.
...
A *source* is a component hosted in Sesam that exposes a stream of entities. Typically, this stream of entities will be the rows of data in an SQL database table, the rows in a CSV file or JSON data from an API.
...
Some sources can accept an additional parameter called an 'offset' token. This token is used to fetch only the entities that have changed since that given offset (continuation support). This can be used to ask for only the entities that have changed since the last time. An offset is an opaque token that may take any form; it is interpreted by the data source only. For example; for a SQL data source it might be a datestamp or for a log based source it might be a location offset.

Sesam provides a number of out-of-the-box *source* types, such as SQL and LDAP. It is also easy for developers to expose a microservice that can supply data from a remote service. The built-in remote data source is able to consume data from these endpoints. These custom data providers can be written and hosted in any language.

To help with this there are a number of template projects hosted on our repository `GitHub <https://github.com/sesam-community>`_ to make this process as easy as possible.
...
A data *sink* is a component that can consume entities fed to them by a pump. The sink has the responsibility to write these entities to the target and handle transactional boundaries. It can also manage batching of multiple entities if supported by the target system.

Several types of data sinks (SQL Sink for example) are available. The use of the JSON push sink enables entities to be pushed to custom microservices or other Sesam service instances.
...
It is also capable of rescanning the data source from first entry, or at configurable points in time. If errors occur during reading or writing of entities, it will keep a log of the failed entities and in the case of write-errors it can retry writing an entity at a later time.
...
DTL has a simple syntax and model where the user declares how to construct a new data entity. It has commands such as 'add', 'copy', and 'merge' which can be applied to properties, list of values and complete entities.
...
In general, DTL transformations are applied to the entities in a dataset through the pipe, and the resulting entities are pushed into a sink that writes to a new dataset. The new dataset is then used as a source for sinks that write the data to external systems.
...
As well as the API there is a UI for working with Sesam. The UI exposes the pipes, datasets, systems and operational information for a service instance.
...
After we have installed Sesam client via pip, we need to configure it. You can read about this below.
