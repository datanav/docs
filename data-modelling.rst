...
There is one important thing to be aware of when changing the value of the '_id' field. Any transforms or operations prior to the change of the '_id', will adhere to the original identity of the entity. This means for instance that if you add a filter to a DTL transform before changing the identity, the filter will apply to the old identity, which may result in the entity not being filtered out if a change occur in the original entity. Since the entity is stored in the dataset with the new identity, but the entity processed is filtered out with the original identity.
...
If it is necessary to create a new '_id' for an entity it should then be created before any filters are applied to the transform and no values from joines entities should be used as part of the new identity.
